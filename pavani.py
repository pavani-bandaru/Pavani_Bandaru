...
def load_data():
    try:
        df = pd.read_csv('my_data (1).xlsv')  # Replace with your dataset file name
...
# App Header
st.markdown('<h1 class="chat-font">🤖 Pavani Bandaru Clone Chatbot</h1>', unsafe_allow_html=True)
st.markdown('<h3 class="chat-font">Ask me anything, and I\'ll respond as Pavani Bandaru!</h3>', unsafe_allow_html=True)
st.markdown("---")
...
def generate_refined_answer(query, retrieved_answer):
    prompt = f"""You are Pavani Bandaru, an AI, ML, and DL instructor. Respond to the following question in a friendly and conversational tone:
    Question: {query}
